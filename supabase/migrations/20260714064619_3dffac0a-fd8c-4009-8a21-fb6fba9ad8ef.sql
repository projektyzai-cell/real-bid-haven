CREATE OR REPLACE FUNCTION public.sign_lease_with_dates(_transaction_id uuid, _start_date date, _end_date date)
 RETURNS text
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
DECLARE v_txn RECORD; v_listing RECORD; v_role text; v_completed_count int; v_bonus int;
BEGIN
  ...
  THEN
    ...
    -- Ecosystem bonus: +9 for the 1st completed StaySafe rental as tenant, +6 for the 2nd. Cap at +15 total.
    SELECT COUNT(*) INTO v_completed_count
      FROM public.lease_transactions
     WHERE tenant_id = v_txn.tenant_id AND state = 'completed';
    v_bonus := CASE WHEN v_completed_count = 1 THEN 9
                    WHEN v_completed_count = 2 THEN 6
                    ELSE 0 END;
    IF v_bonus > 0 THEN
      UPDATE public.profiles
         SET passport_score = LEAST(100, COALESCE(passport_score, 0) + v_bonus)
       WHERE id = v_txn.tenant_id
         AND passport_serial IS NOT NULL;
    END IF;
    ...
    IF v_txn.chat_id IS NOT NULL THEN
      ...
      VALUES (v_txn.chat_id, NULL, 'lease_completed', true,
              jsonb_build_object('event','lease_completed','start', _start_date, 'end', _end_date,
                                 'staysafe_completed_count', v_completed_count,
                                 'passport_bonus', v_bonus));
    END IF;
