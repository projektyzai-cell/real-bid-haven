...
-- 1) Add "party acceptance" (accept being a party to the contract) columns to rental_chats
ALTER TABLE public.rental_chats
  ADD COLUMN IF NOT EXISTS tenant_party_accepted_at timestamptz,
  ADD COLUMN IF NOT EXISTS landlord_party_accepted_at timestamptz;

-- 2) Allow landlord to report payment delay immediately (remove 72h wait).
-- Tenant still receives the systemic 72h warning via chat message.
